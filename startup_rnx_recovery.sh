...
# 5. Move processed bin files to archive
echo "Archiving processed bin files..."
mv "$RAW_DIR"/*."$binEXT" "$FINAL_DIR_BIN" # move bin to archive 

# Cleanup temp files
rm -rf "$TEMP_DIR"

echo "Recovery complete. All 15-min files (contained more than: ${nSEC2KEEP} sec) are cleaned and ready."
